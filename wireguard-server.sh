...
function checkVirt() {
	if [ "$(systemd-detect-virt)" == "openvz" ]; then
		echo "OpenVZ n'est pas supporte."
		exit 1
	fi

	if [ "$(systemd-detect-virt)" == "lxc" ]; then
		echo "LXC n'est pas supporte."
		exit 1
	fi
}
...
function installQuestions() {
	echo "Bienvenue dans le programme d'installation de WireGuard."
	echo ""
	...
	read -rp "Adresse IPv4 ou IPv6 publique : " -e -i "${SERVER_PUB_IP}" SERVER_PUB_IP
